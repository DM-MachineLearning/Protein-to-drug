...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model:int, h:int, dropout:float)->None:
        super().__init__()
        assert d_model % h == 0, "d_model must be divisible by num heads"
        self.d_model = d_model
        self.h = h
        # Use PyTorch's built-in MultiheadAttention
        self.mha = nn.MultiheadAttention(embed_dim=d_model, num_heads=h, dropout=dropout, batch_first=True)

    def forward(self, q, k, v, mask):
        # q, k, v: (batch, seq_len, d_model)
        attn_output, _ = self.mha(q, k, v, attn_mask=None, key_padding_mask=None)
        return attn_output
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        normed = self.norm(x)
        return x + self.dropout(sublayer(normed))
